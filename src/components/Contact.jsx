...
const Contact = () => {
  return (
    <section id="contact" className="py-10 mt-3">
      <div className="max-w-[1120px] mx-auto px-5">
        <div className="h-[400px] bg-footer-bg rounded-2xl p-10 md:p-16 flex flex-col md:flex-row items-center justify-between gap-12 relative">
          <div className="md:w-3/5 text-white">
            <h2 className="text-5xl font-primary_font font-semibold">
            ...
            <div className="mt-8 flex w-full max-w-md p-1 rounded-lg border border-gray-700 bg-gray-800/50 relative">
              <input type="email" placeholder="You're one email away" className="w-full bg-transparent py-2.5 pl-4 pr-36 text-white placeholder-gray-400 focus:outline-none" />
              <button type="submit" className="absolute right-1 top-1 bottom-1 rounded-md bg-primary px-5 text-base font-semibold text-white transition-all duration-300 hover:opacity-90">
                Make It Happen
              </button>
            </div>
          </div>
          <div className="mt-8 md:mt-0 md:absolute md:top-1/2 md:right-0 md:-translate-y-1/2 md:translate-x-1/8">
            {/* KEY CHANGE: Applying the new hover:animate-wiggle class */}
            <PaperPlaneSvg className="w-72 h-72 md:w-[26rem] md:h-[26rem] hover:animate-wiggle" />
          </div>
        </div>
      </div>
    </section>
